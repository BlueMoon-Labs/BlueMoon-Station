...
//
// Это объективная половина проверки. Субъективную делает глайд-полигон.

/// ckey -> /datum/movement_probe
GLOBAL_LIST_EMPTY(movement_probes)

/// Сколько раз скорость пересчитывалась, пока шаг игрока был в полёте.
GLOBAL_VAR_INIT(movement_reschedule_checks, 0)
/// Из них сколько раз расписание удалось подтянуть. Остальные - это либо
/// замедление (шаг уже оплачен, дорожать задним числом ему нельзя), либо
/// пересчёт, который после выравнивания по тику дал ту же самую цену.
