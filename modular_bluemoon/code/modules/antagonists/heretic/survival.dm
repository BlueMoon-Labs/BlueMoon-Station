...
/datum/action/item_action/heretic_escape
	...
	desc = "Разбейте клинок в руке, чтобы перенестись в случайное безопасное место. Клинок будет потерян. Применяйте до оглушения или потери сознания; запрет телепортации мешает побегу."
