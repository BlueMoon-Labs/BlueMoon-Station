...
/datum/heretic_path/void
	...
	weaknesses = "Для поддержания скованности нужно попадать способностями или держать врага в поле. Антимагия блокирует воздействие."
	knowledge = list(
...
/datum/heretic_path/blade
	...
	strengths = "Парирование, быстрое сближение, круговой разрез и пополнение Темпа обычными ударами."
	weaknesses = "Для парирования вторая рука должна быть пустой или держать хватку; стойка короткая, а каждый удар и снаряд расходует её запас блоков."
	knowledge = list(
...
/datum/heretic_path/moon
	...
	weaknesses = "Двойники бьют слабее клинка и разбиваются от пары ударов или выстрелов; обмен требует видимости и свободного пола."
	knowledge = list(
