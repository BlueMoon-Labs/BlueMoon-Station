...
/datum/heretic_path/blood
	...
	strengths = "Быстрое накопление долга, притяжение и лечение ушибов и ожогов за фактический урон врагу. Взыскание восполняет кровь и временно вдвое ослабляет кровотечение. Вознёсшийся удваивает кровотечение врагов в трёх клетках и, проходя по свежей чужой крови на полу, выпивает её: 3 лечения за лужу, не чаще раза в секунду."
	weaknesses = "Чтобы взыскать долг, нужно удержать прямую видимость в пяти клетках в течение секунды. Укрытие или оглушение срывает взыскание; антимагия разрывает связь. Вознёсшегося держите дальше трёх клеток, перевязывайте, зашивайте или прижигайте раны и убирайте кровь с пола: удваивается только то, что сейчас кровоточит, а чистый пол его не лечит."
	knowledge = list(
...
/datum/eldritch_knowledge/base_blood/proc/pact(mob/living/user)
	...
	if(!can_use(user) || QDELETED(required))
		ability_failure = "Договор недоступен: нужно изучить его и находиться в своём теле еретика."
		return FALSE
	if(!invest(user, seals.Copy(), HERETIC_BLOOD_PACT_PAYMENT, renew = TRUE, allow_unlinked = TRUE))
		ability_failure = can_use(user) ? "Для Договора слишком мало здоровья: новая рана оставит меньше безопасного запаса." : "Оплата Договора прервана: вы больше не можете действовать."
		return FALSE
...
/datum/status_effect/heretic_blood_seal/proc/on_owner_death(datum/source)
	SIGNAL_HANDLER
	link_end_reason = source == owner ? "должник умер" : "утрачено знание взыскания"
	qdel(src)
...
/datum/status_effect/heretic_blood_seal/proc/validate_link()
	...
	if(!isturf(owner.loc) || owner.z != blood.blood_body.z)
	...
	if(!blood.valid_victim(blood.blood_body, owner))
		if(collecting)
			if(!blood.line_clear(blood.blood_body, owner))
...
/obj/effect/proc_holder/spell/pointed/heretic_blood/release/can_target(atom/target, mob/user, silent)
	...
	var/datum/antagonist/heretic/heretic = IS_HERETIC(user)
	var/datum/eldritch_knowledge/base_blood/blood = heretic.get_knowledge(/datum/eldritch_knowledge/base_blood)
	if(!heretic_check(user, seal || length(blood.seals) < blood.link_limit, silent, "Связи заняты: выберите прежнего должника, чтобы взыскать его долг."))
		return FALSE
	if(!heretic_check(user, !seal || seal.blood_ref?.resolve() == blood, silent, "Эта связь принадлежит другому еретику. Выберите другого противника."))
		return FALSE
...
/obj/effect/proc_holder/spell/pointed/heretic_blood/lance/can_target(atom/target, mob/user, silent)
	...
	var/datum/status_effect/heretic_blood_seal/seal = victim.has_status_effect(/datum/status_effect/heretic_blood_seal)
	var/datum/antagonist/heretic/heretic = IS_HERETIC(user)
	if(!heretic_check(user, !seal || seal.blood_ref?.resolve() == heretic.get_knowledge(/datum/eldritch_knowledge/base_blood), silent, "Эта связь принадлежит другому еретику. Выберите другого противника."))
		return FALSE
...
/obj/effect/proc_holder/spell/pointed/heretic_blood/coronation/can_target(atom/target, mob/user, silent)
	...
	if(!heretic_check(user, blood.can_use_ascension(user), silent, "Сначала завершите вознесение пути Крови."))
		return FALSE
	if(!heretic_check(user, seal?.blood_ref?.resolve() == blood, silent, "На цели нет вашей кровной связи. Сначала свяжите её Хваткой или способностью «Связать / взыскать»."))
		return FALSE
...
/datum/eldritch_knowledge/base_blood/on_mansus_grasp(atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(!heretic || !proximity_flag || !istype(target, /obj/effect/decal/cleanable/blood) || !isturf(target.loc))
		return FALSE
	grasp_failure_reason = heretic.deed_error(null)
	...
	if(!length(signatures))
		grasp_failure_reason = "В пятне нет чужой подписи крови: собственная кровь и следы без ДНК не подходят."
		return FALSE
	for(var/index in 1 to length(signatures))
		if(!heretic.advance_deed(signatures[index], null, silent = TRUE))
			continue
		stain.name = heretic.deed.trace_name
		stain.desc = heretic.deed.trace_desc
		return TRUE
	grasp_failure_reason = "Все чужие подписи в этом пятне уже изучены. Найдите кровь другого человека."
	return FALSE
...
/datum/eldritch_knowledge/blood_mark
	name = "Метка отсрочки"
	desc = "Хватка оставляет метку на 15 секунд. Удар багровым клыком разбивает её, добавляет 6 долга и продлевает связь на 5 секунд, максимум до 20 секунд от текущего момента."
...
/datum/eldritch_knowledge/final_eldritch/blood_final
	...
	desc = "После трёх назначенных душ принесите на руну три человеческих трупа. Станция узнает место обряда и получит 30 секунд, чтобы помешать. Вы получаете общую стойкость вознесения и можете держать три связи по 30 долга. Враги в трёх клетках от вас в поле зрения кровоточат вдвое сильнее: каждые 2 секунды теряют ещё столько же крови, сколько от своих ран. Удваивается только то, что сейчас кровоточит: перевязанная, зашитая или прижжённая рана ничего не добавляет. Если вы ранены, свежая чужая лужа крови под ногами исчезает и лечит вам 3 урона, сначала ушибы, затем ожоги, не чаще раза в секунду. Засохшая кровь, ваша собственная кровь, следы обуви и кровь ксеноморфов не годятся. «Кровный приговор» раз в 30 секунд переносит долги других связей в выбранную до предела 30, затем начинает её взыскание. Перенос сохраняет общий долг, а жертва получает секунду предупреждения. Смерть снимает эти усиления, оживление возвращает."
