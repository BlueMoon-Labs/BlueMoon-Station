...
/proc/heretic_containment_reason(mob/living/user)
	...
	if(heretic_mindshield_suppressed(user))
		return "Щит разума перекрывает зов Мансуса. Пока имплант в теле, магия и побег клинком недоступны."
	return null
...
/datum/eldritch_knowledge/unshielded_mind
	...
	desc = "Щит разума больше не глушит ваш зов: с имплантом защиты разума магия и побег клинком остаются доступны. Наручники и смирительная рубашка по-прежнему сковывают вас. Вознёсшегося щит не держит и без этого знания."
	passive_desc = "Имплант защиты разума не подавляет вашу магию и побег клинком."
...
/obj/item/implant/mindshield/implant(mob/living/target, mob/user, silent = FALSE)
	...
	if(heretic_mindshield_suppressed(target))
		target.visible_message(span_warning("Сопротивление под кожей [target] стихает: щит разума встал на место."), span_userdanger("Щит разума смыкается над вашим сознанием, и зов Мансуса глохнет. Пока имплант в теле, магия и побег клинком недоступны; извлечь его можно хирургически."))
	else
...
/obj/item/implant/mindshield/get_data()
	return ..() + "<BR><b>Против культов:</b> пока имплант в теле, последователь запретных путей (еретик) не может колдовать и уходить через разбитый клинок. Такой носитель сопротивляется: ввод занимает заметно дольше. Вознёсшегося и самых опытных последователей щит не удерживает."

/obj/item/implanter/mindshield/examine(mob/user)
	. = ..()
	. += span_notice("Щит разума глушит магию еретиков, пока имплант в теле. Еретик сопротивляется вводу, поэтому держите его скованным; сам себе он щит не введёт. Вознёсшегося и самых опытных еретиков щит не удерживает: для них надёжнее наручники.")
