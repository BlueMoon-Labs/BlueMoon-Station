...
/datum/heretic_deed/blade
	next_step = "Способностью «Вызов» бросьте вызов человеку, которого ещё не вызывали: не дальше 5 клеток и когда вы оба 10 секунд без урона."
	name = "Вызовы"
	desc = "Бросайте вызов на дуэль членам экипажа способностью «Вызов». Каждый новый человек засчитывается один раз, принятый вызов - за два шага."
	craft_wait_place = "вызов нового человека"
	craft_wait = "не бросается"
	hint = "Вызванный видит окно с вашим именем - это улика. Отказ или молчание 20 секунд ничего не дают, но этого человека снова можно вызвать только через 5 минут. Принятый вызов уводит вас обоих в изнанку, где чужие не мешают, а на месте вызова остаётся разрыв."
...
/datum/heretic_deed/moon
	...
	hint = "Двойник один, новый заменяет старого. Он стоит до 10 минут, бродит в двух клетках, при осмотре выглядит как вы и по «Голосу двойника» говорит за вас. Внимательный заметит, что он не моргает; 30 урона, нулевой жезл или вспышка рядом рассеивают его."
...
/datum/heretic_deed/blood
	...
	name = "Чужие подписи"
	desc = "Читайте Хваткой Мансуса в намерении «Помощь» свежую чужую кровь: пятно на полу, окровавленный предмет, шприц или пакет с кровью. Каждый человек засчитывается один раз и лечит 5 ушибов. Долг накапливается только на связанных противниках."
	craft_wait_place = "кровь ещё не прочитанного человека"
	craft_wait = "не читается"
	hint = "Лазарет, арена и коридоры после драк полны чужих подписей. Прочитанная кровь на 3 минуты даёт след к её владельцу, а пятно на полу становится вашей меткой: держится три метки, экипаж может заметить, что кровь не засыхает, а уборка или нулевой жезл снимают метку."
