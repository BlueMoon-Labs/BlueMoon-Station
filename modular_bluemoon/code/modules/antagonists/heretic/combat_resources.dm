...
/obj/effect/proc_holder/spell/self/heretic_power/void
	name = "Зимний предел"
	desc = "Потратьте осколок зимы: создайте область холода 5×5 на 15 секунд. Противники замедляются и теряют голос, пока остаются внутри. На руне внутри своего поля можно изготовить клинок Пустоты без охлаждения воздуха. Одновременно существует одна область."
