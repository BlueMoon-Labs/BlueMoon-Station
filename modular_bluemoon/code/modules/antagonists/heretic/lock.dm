...
/datum/heretic_path/lock
	...
	craft_summary = "Хваткой в «Помощи» по шлюзу сделайте его своим порогом: до 4, шлюз в новом отделе - шаг дела пути."
	capture_summary = "Замок на руках: цели охоты 12 секунд наручников, прочим 3 секунды пут; у порога сердце уводит её в изнанку."
	escape_summary = "Ритуальный ключ за 1 ключ и секунду на месте переносит к другому вашему порогу в 30 клетках."
	...
	weakness_points = list(
		...
		"Призрачные наручники срезают кусачки и снимает жезл, их растолкают за 2 секунды, вырваться - за 8 секунд.",
...
/datum/eldritch_knowledge/base_lock
	...
	details = list(
		"«Запечатать проход»: за 1 ключ печать на свободном полу в 5 клетках на 30 секунд, прочность 60.",
...
/datum/eldritch_knowledge/lock_grasp
	...
	details = list(
		"В «Обезоружить» или «Захвате» Хватка открывает соседний закрытый шлюз, поднимая болты.",
...
/datum/eldritch_knowledge/lock_key
	...
	details = list(
		...
		"Выход дальше 30 клеток, заваренный или на болтах, занятый проём, запрет телепортации и другой уровень не пускают.",
		"По-старому: две метки ключом в «Помощи» у шлюзов в 20 клетках, переход с метки за 2 секунды.",
...
/datum/eldritch_knowledge/spell/lock_shackles
	name = "Замок на руках"
	summary = "Цели охоты 12 секунд призрачных наручников, прочим 3 секунды пут на ногах; сбитой цели или цели в печати."
	details = list(
		"Цель в 3 клетках на виду: сбитая с ног, обессиленная, в клетке вашей печати или в середине Замкнутого двора.",
		"Полсекунды замок виден заранее: ушла цель с клетки - он рассыпается, перезарядка возвращается. Сон и отдых не в счёт.",
		...
		"Цель охоты у своего порога сердце за секунду уводит за дверь, в изнанку.",
		"Кусачки и нулевой жезл снимают замок сразу, товарищ растолкает за 2 секунды; защита от магии спасает от него.",
		"После выхода цель минуту невосприимчива к замку и 15 секунд - к любому захвату. Перезарядка 40 секунд.",
	)
...
/datum/status_effect/heretic_lock_shackles/on_remove()
	if(applied)
		...
		heretic_capture_release(owner, HERETIC_LOCK_CAPTURE)
	var/datum/eldritch_knowledge/base_lock/knowledge = lock_ref?.resolve()
	knowledge?.shackles -= src
	lock_ref = null
	return ..()
...
/datum/eldritch_knowledge/spell/lock_release
	...
	details = list(
		...
		"Одиночное размыкание не возвращает ключ и перезаряжается 15 секунд, общих с добычей ключа ключом.",
	)
	role = HERETIC_ROLE_ATTACK
...
/datum/eldritch_knowledge/spell/lock_court
	...
	details = list(
		"2 секунды будущие печати видны заранее; занятые клетки пропускаются.",
...
/datum/eldritch_knowledge/final_eldritch/lock_final
	...
	details = list(
		"Три назначенные души и три человеческих трупа; обряд раскрывает своё место и даёт станции 30 секунд.",
