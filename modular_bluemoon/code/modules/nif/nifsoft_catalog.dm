...
/datum/computer_file/program/nifsoft_downloader/ui_data(mob/user)
	...
	var/obj/item/card/id/paying_card = card_slot?.GetID()
	paying_account = paying_card?.registered_account || null
	data["paying_account"] = paying_account
	data["current_balance"] = paying_account?.account_balance
...
/datum/computer_file/program/nifsoft_downloader/ui_static_data(mob/user)
	...
	if(!ishuman(nif_user))
		...
		var/obj/item/organ/cyberimp/brain/nif/user_nif = nif_user.getorgan(/obj/item/organ/cyberimp/brain/nif)
		if(!user_nif)
			target_nif = null
