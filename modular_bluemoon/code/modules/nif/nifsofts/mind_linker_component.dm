/// A component that handles linking multiple player's minds into one network.
...
/datum/component/mind_linker
	...
	/// Does the component give an action to speak? By default, yes
	var/speech_action = TRUE
	/// Does the component check if the person being linked has a mindshield or anti-magic?
...
/datum/component/mind_linker/Initialize(
	...
	src.speech_action_overlay_state = speech_action_overlay_state
...
/datum/component/mind_linker/proc/link_mob(mob/living/to_link)
	...
	RegisterSignal(to_link, COMSIG_LIVING_DEATH, PROC_REF(sig_unlink_mob))
...
/datum/component/mind_linker/proc/unlink_mob(mob/living/to_unlink)
	...
	UnregisterSignal(to_unlink, COMSIG_LIVING_DEATH)
...
/// Signal proc sent from any signals given to us in initialize.
...
/// Subtype of mind linker which is more active rather than passive,
...
/datum/component/mind_linker/active_linking/unlink_mob(mob/living/to_unlink)
	...
	for(var/mob/living/other_link as anything in linked_mobs)
		to_chat(other_link, span_warning("You feel a presence disappear from [owner.real_name]'s [network_name]."))
