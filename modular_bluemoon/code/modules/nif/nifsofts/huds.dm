...
/datum/nifsoft/hud/proc/remove_huds()
	if(hud_type)
		...
		hud.remove_from_hud(linked_mob)

	for(var/trait in hud_traits)
		REMOVE_TRAIT(linked_mob, trait, GLASSES_TRAIT)
...
/datum/nifsoft/hud/activate()
	...
	if(!active)
		...
		if(eyewear_check)
			...
			UnregisterSignal(worn_glasses, COMSIG_ITEM_POST_UNEQUIP)

		return TRUE

	add_huds()
	if(eyewear_check)
		RegisterSignal(worn_glasses, COMSIG_ITEM_POST_UNEQUIP, PROC_REF(on_glasses_unequip))

	return TRUE

/// Deactivates the NIFSoft HUD if the compatible glasses are removed while the NIFSoft is active.
/datum/nifsoft/hud/proc/on_glasses_unequip(obj/item/source, mob/unequipper, slot)
	SIGNAL_HANDLER
...
/datum/element/nifsoft_hud/proc/on_examine(datum/source, mob/user, list/examine_text)
	...
	examine_text += span_blue("Wearing this item in your glasses slot will allow you to use NIFSoft HUDs.")
