...
/datum/nifsoft/soul_poem
	...
	/// The cooldown between scans for nearby NIF users.
	var/next_scan_time = 0
	/// How often we scan for new nearby NIF users.
...
/datum/nifsoft/soul_poem/proc/add_message(sender_identifier, received_name, received_message)
	...
	for(var/message in message_list)
		if(message["identifier"] == sender_identifier)
			...
			message["timestamp"] = worldtime2text()
			return TRUE

	message_list.Insert(1, list(list(identifier = sender_identifier, sender_name = received_name, message = received_message, timestamp = worldtime2text())))
	return TRUE
...
/// Periodically scans for nearby NIF users and exchanges poems with them.
/datum/nifsoft/soul_poem/process(seconds_per_tick)
	...
	next_scan_time = world.time + scan_cooldown

	for(var/mob/living/carbon/human/nearby_human in oview(1, linked_mob))
		if(nearby_human == linked_mob)
			continue

		var/datum/nifsoft/soul_poem/nearby_nifsoft = nearby_human.find_nifsoft(/datum/nifsoft/soul_poem)
		if(!nearby_nifsoft || QDELETED(nearby_nifsoft))
			continue

		send_message(nearby_nifsoft)
		nearby_nifsoft.send_message(src)
