...
| `heretic_feedback.dmi` | `ash_aura`, `rust_aura`, `flesh_aura`, `void_aura`, `blade_aura`, `moon_aura`, `cosmic_presence` | Нимбы вознесения 32x32 при 1.5x, 48 x 0.5 дс: кольцо углей с пламенем, ржавое железо с трещинами, кольцо плоти с глазами, ледяное кольцо Пустоты, восемь клинков, серп луны с осколками, созвездие Космоса. |
...
| Файл | Состояния | Описание |
| --- | --- | --- |
| `heretic_hoods_rust_worn.dmi`, `heretic_hoods_cosmic_worn.dmi` | `rust_armor`, `cosmic_armor` | Надетые капюшоны, четыре стороны: бурый куколь в тоне рясы Ржавчины; лиловый венец с камнем над тёмным куколем Космоса. Файлы были пустыми. |
