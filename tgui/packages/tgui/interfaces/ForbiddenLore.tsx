...
const Ledger = ({ showDeed = true }: { showDeed?: boolean }) => {
  ...
  return (
    <article className={`HereticBook__page HereticBook__page--${side}`} aria-label={side === 'left' ? 'Левая страница' : 'Правая страница'}>
      ...
      <div className="HereticBook__pageBody">
        ...
        {side === 'left' && <Ledger showDeed={chapter !== 'Знания'} />}
  ...
  return (
    <>
      <Page side="left" chapter="Охота">
        ...
        <p className="HereticBook__annotation">Активируйте живое сердце в руке, чтобы узнать направление к цели. Alt-ЛКМ по сердцу позволяет сменить цель. Коснитесь сердцем поверженного члена экипажа, чтобы сделать целью его: подойдёт крит, наручники, оглушение или сбивание с ног; добровольно лёгший или уснувший не считается. Смена цели любым способом - не чаще раза в 3 минуты.</p>
        <p className="HereticBook__annotation">Еретики и их слуги не подходят для подношения. Если назначенная цель сама стала еретиком, смените её без ожидания перезарядки.</p>
        <h2>Дело пути</h2>
        <DeedSection />
      </Page>
      <Page side="right" chapter="Охота">
        ...
        <ol className="HereticBook__steps">
          <li>Обезвредьте названную цель. Живую цель достаточно связать наручниками, оглушить или сбить с ног; добровольно лёгший или уснувший не считается. Цель в крите принимается без наручников, даже если ещё стоит. Хватка Мансуса помогает в захвате. Если назначенная цель погибла, подойдёт её труп.</li>
          <li>Коснитесь цели своим живым сердцем: круг проступит прямо под телом. Или перенесите её на руну, положите рядом сердце и выберите «Обряд возвращения». В обоих случаях сохраняйте неподвижность 8 секунд. Живую жертву круг удержит и защитит от кровотечения. Перемещение жертвы или прерывание еретика срывает обряд.</li>
          <li>Живая жертва даёт 2 очка знаний и 1 побочное, проходит через Мансус и возвращается живой в коридор станции не позднее чем через 3 минуты. Труп даёт только 1 очко знаний без побочного и остаётся на месте для возможной реанимации. Оба варианта засчитываются для вознесения. Одна душа принимается лишь однажды, даже после реанимации.</li>
        </ol>
        <h3>Разломы</h3>
  ...
  return (
    <>
      <Page side="left" chapter="Помощь">
        ...
        <p>Каждый разлом можно изучить отдельно от других еретиков: чужое исследование не забирает ваши очки. Новые разломы доступны и тем, кто стал еретиком позднее.</p>
        <h3>След и кристалл завесы</h3>
        <p>После изучения на месте разлома на 3 минуты остаётся видимый след. Экипаж может запечатать его нейтрализатором аномалий, и тогда след схлопывается в кристалл завесы: учёные разбирают его на очки исследований. Сами вы след запечатать не можете, но кристалл можно отнять или украсть. Раздавите его в руке, и 20 секунд он будет затягивать раны и возвращать выносливость. Пока действует один кристалл, второй не сработает.</p>
        <h3>Приготовьте место</h3>
        <p>Кодексом на полу начертите руну. Нужен участок пола 3×3 без стен, космоса, лавы и соседних рун. Столы, баки и предметы не мешают. Рисование занимает 8 секунд. Нажмите на центр руны пустой рукой, чтобы выбрать изученный обряд. Кодексом или Хваткой Мансуса руну можно стереть; заряд хватки при этом не тратится.</p>
        <RitualDiagram />
      ...
      <Page side="right" chapter="Помощь">
        ...
        <h3>Другие еретики</h3>
        <p>Боевая магия еретиков не действует на других еретиков и их слуг. Защита связана со служением Мансусу и действует даже без кодекса. Обычный физический урон, в том числе удар клинком, она не останавливает.</p>
        <p>Еретики и их слуги не подходят для жертвоприношений и финального обряда. Если назначенная цель сама стала еретиком, выберите другую в главе «Охота» или через живое сердце: ждать перезарядки не нужно.</p>
        <h3>Сердце ведёт к жертве</h3>
        <p>В главе «Охота» выберите живую цель. Среди новых предложенных целей по возможности есть кто-то из командования и сотрудник СБ, если они есть на станции. Призовите живое сердце: сожмите его для поиска, Alt-ЛКМ меняет цель. Касание сердцем поверженного члена экипажа делает целью его, если прошла перезарядка смены цели. Обезвредьте цель и коснитесь её сердцем: круг проступит прямо под телом. Руна тоже подходит, если перенести туда человека вместе со своим сердцем. Живую цель достаточно связать, оглушить или сбить с ног; цель без сознания тоже подойдёт. Цель в крите принимается без наручников, даже если ещё стоит. Если назначенный человек погиб, можно принести его труп.</p>
        ...
        <p>На время обряда круг удерживает живую жертву и останавливает кровотечение. Перемещение жертвы или прерывание еретика срывает обряд. В Мансусе живой человек проходит испытание Дома памяти: в лабиринте комнат, где действует особое правило вашего пути, три осколка нужно донести до печати перед вратами. Через 3 минуты Дом отпускает его сам, и он появляется в коридоре станции на глазах у окружающих.</p>
        ...
        <h3>Вознесение</h3>
        <p>За раунд могут вознестись несколько еретиков. Чужое вознесение не закрывает ваш обряд: каждый выполняет его условия самостоятельно.</p>
        <p>Совершите {data.hunt.sacrifices_required} жертвоприношений и изучите последнее знание пути. Для вознесения принесите на руну трупы членов экипажа: {data.hunt.ascension_bodies ?? 3}. Подходят только тела, которыми управлял человек; очеловеченные мартышки и пустые клоны не годятся. Финальный обряд длится 30 секунд; его подношения не возвращаются. Каждая попытка объявляется станции, между началами попыток должно пройти три минуты. Выбранные тела подсвечиваются для вас зелёным.</p>
        <p>После второй жертвы станция получает предупреждение об оккультной угрозе. Вознесение доступно не раньше чем через три минуты после этого предупреждения. Финальный обряд нельзя провести в зоне открытого космоса, даже на своей площадке с воздухом: руна должна лежать в помещении станции или другой локации.</p>
        <h3>Отход и плен</h3>
        <p>Разбитый клинок переносит вас в случайное безопасное место; следующий побег возможен не раньше чем через 10 секунд. После вознесения побег клинком закрыт. В наручниках и смирительной рубашке магия Мансуса, обряды на руне и побег клинком недоступны, пока вы скованы. Имплант защиты разума глушит их, пока он в теле: вы сопротивляетесь вводу, но скованного это лишь задержит, а извлечь щит можно только хирургически. Побочное знание Разум за завесой снимает действие щита, а вознёсшегося он не держит вовсе. Станция знает об этом из предупреждения об угрозе.</p>
        <h3>Дело пути</h3>
