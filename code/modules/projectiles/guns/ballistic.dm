/obj/item/gun/ballistic
	...
	var/SFX_GUN_INSERT_FULL_MAGAZINE = "gun_insert_full_magazine"
	var/SFX_GUN_INSERT_EMPTY_MAGAZINE = "gun_insert_empty_magazine"
	var/SFX_GUN_BOLT_UNLOCK = "gun_slide_lock"
	var/SFX_GUN_REMOVE_MAGAZINE = 'sound/weapons/gun_magazine_remove_full.ogg'
	var/SFX_GUN_REMOVE_EMPTY_MAGAZINE = "gun_remove_empty_magazine"
	var/SFX_GUN_BOLT_LOCK ='sound/weapons/gun_chamber_round.ogg'
		...
		if (!magazine && istype(AM, mag_type))
			if(user.transferItemToLoc(AM, src))
				...
				if(magazine.ammo_count())
					playsound(src, SFX_GUN_INSERT_FULL_MAGAZINE, 70, 1)
					if(!chambered)
						chamber_round()
						addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(playsound), src, SFX_GUN_BOLT_LOCK, 100, 1), 3)
				else
					playsound(src, SFX_GUN_INSERT_EMPTY_MAGAZINE, 70, 1)
				A.update_icon()
				update_icon()
				return TRUE
			else
