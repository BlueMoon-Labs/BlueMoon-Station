...
/obj/machinery/skill_station/proc/remove_skillchip(obj/item/skillchip/to_be_removed)
	...
	if(to_be_removed.removable)
		if(!carbon_occupant.put_in_hands(to_be_removed))
			to_be_removed.forceMove(drop_location())
	else
		qdel(to_be_removed)

	to_chat(occupant, span_notice("Operation complete!"))
	...
	switch(action)
		...
		if("eject")
			...
			if(inserted_skillchip)
				...
				var/mob/living/carbon/carbon_occupant = occupant
				if(!carbon_occupant.put_in_hands(inserted_skillchip))
					inserted_skillchip.forceMove(drop_location())
				inserted_skillchip = null
				return TRUE
		if("toggle_activate")
			var/chipref = params["ref"]
