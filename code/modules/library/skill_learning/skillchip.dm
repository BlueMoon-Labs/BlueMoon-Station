 ...
 * * force - Boolean. Whether or not to just force de-activation if it would be prevented for any reason.
 * * brain_owner - the owner var of the brain is set to null on organ/Remove(), so we need this if owner is null.
 */
...
/**
 * Clears all references this chip has to a brain and its owner.
 *
 * Safe to call outside of removal flows (e.g. when the chip is qdel'd while
 * still inside a brain): deactivates it and drops it from the brain's chip list.
 * Arguments:
 * * brain - The brain the chip is/has been implanted in.
 */
/obj/item/skillchip/proc/clear_skillchip_references(obj/item/organ/brain/brain)
	LAZYREMOVE(brain.skillchips, src)
	COOLDOWN_RESET(src, chip_cooldown)
	if(!QDELETED(brain.owner) && active)
		try_deactivate_skillchip(silent = TRUE, force = TRUE)
	holding_brain = null

/obj/item/skillchip/Destroy()
	var/obj/item/organ/brain/holding = holding_brain
	if(holding)
		clear_skillchip_references(holding)
	return ..()
