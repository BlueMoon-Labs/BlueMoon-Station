...
//
//isappearance() тут не нужен и вреден: аппиранс - не датум, istype(аппиранс, /datum) даёт ноль,
//так что isdatum() его и так не ловит. Зато сам макрос отвечает TRUE на любой голый /datum:
//istype() принимает инстанс вторым аргументом за его тип, а /image наследуется от /datum, то
//есть istype(картинка-детектор, новый /datum) - правда. Голый датум шаблона проезжал в копию
//как "аппиранс"
