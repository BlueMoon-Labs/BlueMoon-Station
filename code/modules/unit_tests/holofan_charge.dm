/// У собранного вентилятора есть предохранительная муфта, которая распахивается
/// от перепада давления. У голограммы её нет, поэтому проектор давал бесплатные
/// вечные стены, держащие любой перепад. Ограничение - время: эмиттер тратит
/// заряд, пока фаны развёрнуты, и гасит их, когда заряд кончился.
///
