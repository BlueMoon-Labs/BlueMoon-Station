...
/datum/unit_test/heretic_resource_hud_changes/Run()
	...
	TEST_ASSERT_EQUAL(indicator.displayed_value, 1, "HUD показывает начальный Темп.")
	TEST_ASSERT(knowledge.spend_combat_resource(), "Начальный Темп можно потратить.")
...
/datum/unit_test/heretic_resource_hud_transfer/Run()
	...
	TEST_ASSERT_EQUAL(new_indicator.displayed_value, 3, "Смена тела сохраняет накопленные угольки.")
