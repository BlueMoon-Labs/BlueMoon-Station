...
/datum/unit_test/space_weather_solar_multiplier/Run()
	...
	TEST_ASSERT(GLOB.solar_output_multiplier > 1, "Ионная буря на пике не подняла выработку панелей ([GLOB.solar_output_multiplier]) - награды за подготовку нет")
	blizzard.cleanup()
	TEST_ASSERT_EQUAL(GLOB.solar_output_multiplier, 1, "Ионная буря ушла, оставив множитель выработки [GLOB.solar_output_multiplier]")
	qdel(blizzard)
	...
	TEST_ASSERT(GLOB.solar_output_multiplier < 1, "Поле микрообломков на пике не посадило выработку ([GLOB.solar_output_multiplier]) - осевшая пыль ни на что не влияет")
	// Админская отмена ходит мимо end(): множитель обязан вернуться и на этом пути.
	debris.kill()
	TEST_ASSERT_EQUAL(GLOB.solar_output_multiplier, 1, "Убитое поле микрообломков оставило множитель выработки [GLOB.solar_output_multiplier] до конца раунда")
	qdel(debris)
...
/datum/unit_test/space_weather_bluespace_restore/Run()
	...
	TEST_ASSERT(GLOB.bluespace_teleport_noise > 0, "Шторм на пике не расшатал точность телепортации - риска в событии нет")
	storm.kill()
	TEST_ASSERT_EQUAL(GLOB.bluespace_teleport_noise, 0, "Убитый шторм оставил разброс телепортации [GLOB.bluespace_teleport_noise] до конца раунда")
	qdel(storm)
	...
	TEST_ASSERT(GLOB.bluespace_teleport_blocked, "Интерфаза на пике не заглушила блюспейс - событие ничего не делает")
	interphase.on_phase_enter(PHENOMENON_PHASE_DEPARTURE, PHENOMENON_PHASE_PEAK)
	TEST_ASSERT(!GLOB.bluespace_teleport_blocked, "Интерфаза держит блюспейс заглушенным и после пика")

	// И тот же путь через админскую отмену, мимо end().
	interphase.on_phase_enter(PHENOMENON_PHASE_PEAK, PHENOMENON_PHASE_APPROACH)
	interphase.kill()
	TEST_ASSERT(!GLOB.bluespace_teleport_blocked, "Убитая интерфаза оставила станцию без телепортации до конца раунда")
	qdel(interphase)
...
/datum/unit_test/space_weather_interphase_evac_gate/Run()
	...
	TEST_ASSERT(!control.can_fire(null), "Интерфаза согласилась запуститься при вызванном шаттле")
