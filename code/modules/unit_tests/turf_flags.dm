...
/datum/unit_test/turf_flag_defaults/Run()
	...
	// Световые флаги производные: типовым дефолтом их не ставят нигде.
	var/turf/base_path = /turf
	var/turf/open/space/space_path = /turf/open/space
	TEST_ASSERT_EQUAL(initial(base_path.lighting_flags), NONE, "световые флаги обязаны быть производными, а не типовым дефолтом")
	TEST_ASSERT_EQUAL(initial(space_path.lighting_flags), NONE, "световые флаги обязаны быть производными, а не типовым дефолтом")
