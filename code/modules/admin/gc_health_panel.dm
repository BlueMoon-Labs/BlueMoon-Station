...
/client/proc/cmd_gc_health_help()
	...
	output += "<td>Агрессивный бюджет ([GC_HARDDEL_LOBBY_BUDGET_MS]мс), удаляет по [GC_HARDDEL_LOBBY_MAX_PER_FIRE] объектов за тик. Использует свободное CPU время до старта раунда.</td></tr>"
	output += "<tr><td><b>HOLD</b></td><td>Очередь hard delete небольшая, всё под контролем</td>"
	output += "<td>Минимальный бюджет ([GC_HARDDEL_BUDGET_MIN_MS]мс), удаляет по [GC_HARDDEL_MAX_PER_FIRE] объектов за тик. Экономит ресурсы.</td></tr>"
	output += "<tr><td><b>RECOVER</b></td><td>Очередь растёт, Q3 дельта положительная несколько сэмплов подряд</td>"
	output += "<td>Увеличенный бюджет (до [GC_HARDDEL_BUDGET_MAX_MS]мс), пытается сократить очередь.</td></tr>"
	output += "<tr><td><b>OVERFLOW</b></td><td>Очередь превысила [GC_HARDDEL_OVERFLOW_THRESHOLD] объектов</td>"
	output += "<td>Максимальный бюджет ([GC_HARDDEL_OVERFLOW_BUDGET_MAX_MS]мс), удаляет по [GC_HARDDEL_OVERFLOW_MAX_PER_FIRE] за тик. Аварийный режим.</td></tr>"
	output += "</table>"
