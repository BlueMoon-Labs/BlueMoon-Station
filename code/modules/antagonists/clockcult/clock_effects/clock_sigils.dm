	...
	if(isliving(AM))
		...
		if(L.stat <= stat_affected)
			if((!is_servant_of_ratvar(L) || (affects_servants && is_servant_of_ratvar(L))) && (L.mind || L.has_status_effect(STATUS_EFFECT_SIGILMARK)) && !isdrone(L))
				...
				if(I)
					if(isitem(I))
						L.visible_message("<span class='warning'>[I.name] [L] [resist_string], защищая [L.ru_na()] от эффектов [src]!</span>", \
...
/obj/effect/clockwork/sigil/submission
	...
	clockwork_desc = "Сигил, который порабощает любое существо, не являющееся Слугой, если оно останется на нём в течение 8 секунд. Не проникает через имплант защиты разума."
...
/obj/effect/clockwork/sigil/rite/on_attack_hand(mob/living/user, act_intent = user.a_intent, unarmed_attack_flags)
	...
	if(!GLOB.all_clockwork_rites.len) //Did we already generate the list?
		generate_all_rites()
	if(performing_rite)
		to_chat(user, "<span class='warning'>Кто-то уже проводит ритуал здесь!")
		return
