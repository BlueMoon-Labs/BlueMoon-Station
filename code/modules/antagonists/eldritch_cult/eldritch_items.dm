...
/obj/item/melee/sickly_blade/attack_self(mob/user)
	if(IS_HERETIC(user) || IS_HERETIC_MONSTER(user))
		...
		if(!origin)
			return
		var/turf/safe_turf = is_station_level(origin.z) ? find_heretic_station_turf(for_escape = TRUE) : find_safe_turf(zlevels = list(origin.z), extended_safety_checks = TRUE, dense_atoms = FALSE)
