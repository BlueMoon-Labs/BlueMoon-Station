...
/obj/item/forbidden_book/ui_data(mob/user)
	...
	if(available_target)
		if(target.stat == DEAD)
			...
			target_status = "Цель готова к обряду. Доставьте её и своё живое сердце к руне: 2 очка знаний и 1 побочное."
		else
			target_status = "Цель жива. Для обряда подойдут наручники, оглушение или положение лёжа. Цель в крите принимается без наручников."
	data["hunt"] = list(
