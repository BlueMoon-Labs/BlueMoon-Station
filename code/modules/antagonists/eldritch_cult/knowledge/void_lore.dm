...
/datum/eldritch_knowledge/final_eldritch/void_final
	...
	desc = "После трёх подношений принесите три мёртвых тела на руну. Начало обряда раскроет его место всей станции и даст экипажу 30 секунд, чтобы помешать. Вознесение окружает вас зимней бурей и даёт защиту от среды. Последний такт наносит видимым врагам в пределах 5 клеток 30 холодовых ожогов, замедляет на 4 секунды, оттесняет на две клетки, охлаждает и отмечает их, оставляя зимний круг радиусом три клетки на 12 секунд. Аура в пределах 5 клеток поддерживает охлаждение и скованность; голос подавляют только активные зимние поля."
