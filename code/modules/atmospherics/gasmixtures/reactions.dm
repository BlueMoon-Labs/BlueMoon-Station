...
//
// Обе реакции требуют давления выше номинала обычной трубы. Это единственные
// реакции в игре с таким условием, и вся их суть в нём: обычная разводка на
// таком давлении копит усталость и уходит в течь раньше, чем успеет наварить
// хоть сколько-то, а усиленная держит. Ворота работают последствием, а не
// запретом - игрок волен попробовать в обычных трубах и посмотреть, что будет.
...
/datum/gas_reaction/pyronite_formation/react(datum/gas_mixture/air, datum/holder)
	...
	var/plasma_moles = air.get_moles(GAS_PLASMA)
	var/proto_moles = air.get_moles(GAS_PROTO_NITRATE)
	// Давление сверх порога ускоряет синтез: усиленный контур должен окупать
	// вложенные в него трубы, а не просто снимать запрет.
