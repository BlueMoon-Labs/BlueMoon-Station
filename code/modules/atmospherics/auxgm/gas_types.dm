...
// Оба газа ниже нельзя сварить в обычной разводке: их реакции требуют давления
// выше номинала обычной трубы. Облако обоим даёт готовое состояние "generic" -
// новых спрайтов не рисуется, различает их цвет.
/datum/gas/pyronite
	id = GAS_PYRONITE
	specific_heat = 25
	name = "Pyronite"
	description = "Топливо высокого давления: горит вдвое жарче плазмы и питает верхние рецепты кристаллизатора. Варится из трития и плазмы на прото-нитрате при 1500-6000 K и давлении выше полутора номиналов обычной трубы - то есть только в усиленном контуре. Смертельно опасен тем же, чем полезен: попав в помещение вместе с кислородом, выжигает его целиком."
	gas_overlay = "generic"
	color = "#ff7722"
	moles_visible = MOLES_GAS_VISIBLE * 0.5
	flags = GAS_FLAG_DANGEROUS
	...
	// Вершина цепочки: реакция требует давления выше полутора номиналов обычной
	// трубы, то есть газ недоступен без усиленного контура. Отсюда и цена выше
	// заукера - тот при всей своей глубине варится в обычной разводке.
	tier = GAS_TIER_EXOTIC
	price = 22
...
/datum/gas/fluxin
	...
	specific_heat = 45
	name = "Fluxin"
	description = "Катализатор: раздвигает верхнюю границу температурного окна плуоксия, прото-нитрата, хилия и заукера и снижает расход сырья в них - то есть делает дешевле саму глубину. Варится из гелия, BZ и плазмы при 300-600 K и давлении выше полутора номиналов обычной трубы. Медленно тратится в тех реакциях, которым помогает, и не ядовит сам по себе - опасен тем, что уводит чужие реакции туда, где контур к ним не готов."
	gas_overlay = "generic"
	color = "#33ddcc"
	moles_visible = MOLES_GAS_VISIBLE * 0.5
	// Как и пиронит: за усиленным контуром, значит вершина цепочки. Дороже
	// пиронита, потому что удешевляет саму глубину - работает на всю цепочку,
	// а не на один рецепт.
	tier = GAS_TIER_EXOTIC
	price = 24
