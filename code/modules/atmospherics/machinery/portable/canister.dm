...
/obj/machinery/portable_atmospherics/canister/proc/create_gas()
	if(gas_type)
		...
		if(air_temperature <= 0)
			...
			air_temperature = air_contents.return_temperature()
		if(air_temperature <= 0)
			return
		air_contents.set_moles(gas_type,(maximum_pressure * filled) * air_volume / (R_IDEAL_GAS_EQUATION * air_temperature))
...
/obj/machinery/portable_atmospherics/canister/air/create_gas()
	...
	if(air_temperature <= 0)
		air_contents.set_temperature(T20C)
		air_temperature = air_contents.return_temperature()
	if(air_volume <= 0 || air_temperature <= 0)
		return
	air_contents.set_moles(GAS_O2, (O2STANDARD * maximum_pressure * filled) * air_volume / (R_IDEAL_GAS_EQUATION * air_temperature))
	air_contents.set_moles(GAS_N2, (N2STANDARD * maximum_pressure * filled) * air_volume / (R_IDEAL_GAS_EQUATION * air_temperature))
