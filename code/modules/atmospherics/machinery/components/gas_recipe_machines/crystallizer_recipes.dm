...
/datum/gas_recipe
	...
	/// Порог на входе означает буквально "корми через усиленный контур" - это
	/// и есть смысл ворот.
	var/min_pressure = 0
...
// требует давления, которого обычная разводка не держит.
...
// Осколок и раньше стоял за усиленным контуром, теперь этот контур ещё и должен
// в нём что-то сварить, а не просто выдержать давление.
...
// нет намеренно: усиленный контур - это признак третьего уровня, и смешивать
