...
/proc/createRandomZlevel(name = AWAY_MISSION_NAME, list/traits = list(ZTRAIT_AWAY = TRUE), list/potential_levels = GLOB.potential_away_levels)
	...
	// traits только именованным: третий позиционный у load_new_z_level - это ориентация.
	var/datum/space_level/level = load_new_z_level(map, name, ztraits = traits)
	if(!level)
...
/proc/load_away_mission(mission_file, existing_z = null)
	...
	if(existing_z)
		away_mission.load(locate(1, 1, existing_z), centered = FALSE)
		return existing_z

	var/datum/space_level/level = away_mission.load_new_z(ztraits = list(ZTRAIT_AWAY = TRUE), centered = FALSE)
	return level?.z_value
