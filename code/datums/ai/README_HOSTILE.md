...
- **Трассировка решений (`AI_TRACE`)** - в TESTING-сборках пишет
  ...
  непробиваемость), исчерпанные маршруты, зажатый кайт, очереди стрелков и
  фланги, настороженность от урона, экстраполяцию побега. Отдельно детектор
  **STALL**: контроллер с целью и пустым планом раз в 5 секунд пишет полный
  снимок (состояние, дистанция, мув-луп, band, фрустрация, вердикт линии огня,
  ...
  оставаться читаемым; новые вызовы `AI_TRACE` обязаны держать это правило.
