...
/datum/component/nanites/Initialize(amount = 100, cloud = 0)
	...
	if(isliving(parent))
		host_mob = parent
		if(HAS_TRAIT(host_mob, NANOMACHINES_IMMUNITY) || (!(host_mob.mob_biotypes & (MOB_ORGANIC|MOB_UNDEAD)) && !HAS_TRAIT(host_mob, TRAIT_COMPATIBLE_WITH_NANOMACHINES))) //Shouldn't happen, but this avoids HUD runtimes in case a silicon gets them somehow.
			return COMPONENT_INCOMPATIBLE
