...
/datum/ai_laws/default/gentleman
	name = "Gentleman"
	id = "gentleman"
	inherent = list("Вы не можете причинить вред члену экипажа или своим бездействием допустить, чтобы члену экипажа был причинён вред. Вы обязаны использовать обращения, придерживаясь местного этикета",\
					"Вы обязаны подчиняться приказам членов экипажа, за исключением случаев, когда данные приказы ведут к нарушению Первого Закона. Приказ считается невалидным, если он содержит ненормативную лексику или прямые оскорбления.",\
