...
//
/// Рабочее давление обычной трубы. Выше него сеть начинает копить усталость.
/// Штатная разводка живёт на 101 кПа, вентили на 120-140, так что запас
/// стократный. Газовый насос на максимуме (4500) даёт 45% номинала:
/// обычная помпа трубе не враг, и даже прогрев такой линии почти вдвое не
/// доводит её до гула. Это осознанный баланс по итогам полунедели на проде
/// (доклады Moth и FL-432): сток ВСЕХ карт разведён обычной трубой с подачей
/// до 4500, и номинал 3000 заставлял станцию течь саму по себе, без участия
/// игрока. Порог остаётся требованием только к осознанному перегрузу:
/// объёмный насос (потолок = два номинала), сильный нагрев контура, бомбы.
#define PIPE_PRESSURE_RATING_STANDARD		10000
/// Усиленная и теплообменная труба: обе существуют ради горячих контуров.
/// Десятикратный разнос с обычной держится сознательно: прогретый контур,
/// набитый объёмным насосом до потолка, множит давление температурой, и предел
/// обязан оставаться недосягаемым для всего, кроме совсем осознанного безумия.
#define PIPE_PRESSURE_RATING_REINFORCED		100000
/// Уставка сифонов газовых хранилищ - штатного маппового оборудования,
/// которое компрессирует линию само, без игрока.
///
/// Число фиксированное и НЕ масштабируется с номиналом: сифон - идеальный
/// компрессор (давление комнаты в формуле переноса не участвует) и доедет до
/// любой уставки, пока в камере есть газ, - но плотнее набитая линия хранилища
/// даёт лишь чуть больший буфер, а рвётся и вытравливается при поломке
/// пропорционально громче. 4000 - историческая уставка хранилищ, при номинале
/// 10000 у неё запас в два с половиной раза. Правило прежнее: штатное мапповое оборудование не
/// подходит к номиналу вплотную (сторожится юнит-тестом
/// atmos_mapped_vents_respect_pipe_rating).
#define PIPE_SAFE_WORKING_PRESSURE			4000
/// Объём секции на узел. Усиленная платит просветом за толщину стенки.
#define PIPE_VOLUME_PER_NODE_STANDARD		35
#define PIPE_VOLUME_PER_NODE_REINFORCED		20
/// Во сколько раз давление должно перекрыть номинал, чтобы дойти до течи...
#define PIPE_STRESS_LEAK_RATIO				1.3
/// ...и до разрыва стыка. Газовый насос упирается в 0.45 номинала и не может
/// даже продавить течь; объёмный доезжает до двух номиналов и рвёт.
#define PIPE_STRESS_RUPTURE_RATIO			1.8
/// Стадии повреждения трубы.
#define PIPE_DAMAGE_INTACT					0
#define PIPE_DAMAGE_LEAK					1
#define PIPE_DAMAGE_RUPTURE					2
/// Усталость, при которой стадия растёт на единицу.
#define PIPE_FATIGUE_MAX					100
/// Прирост за проход свипа: (stress - 1) * SCALE, но не больше CAP. Потолок
/// гарантирует минимум PIPE_FATIGUE_MAX / CAP проходов до отказа даже при
/// абсурдном давлении - мгновенных разрывов не бывает никогда.
#define PIPE_FATIGUE_GAIN_SCALE				4
#define PIPE_FATIGUE_GAIN_CAP				20
/// Убыль за проход, когда давление вернулось под номинал.
#define PIPE_FATIGUE_DECAY					5
/// Литры газа в турф за проход при полном перепаде давления.
#define PIPE_LEAK_LITRES					4
#define PIPE_RUPTURE_LITRES					20
/// Как часто напряжённая сеть подаёт голос в комнату.
#define PIPE_STRESS_MESSAGE_COOLDOWN		(15 SECONDS)
/// Как часто пробитая труба шипит и парит, пока травит газ. Свищ под полом
/// невидим и не варится, не вскрыв плитку, - звук и пар это ЕДИНСТВЕННЫЙ
/// способ его найти, без них "газы просто пропадают из труб".
#define PIPE_BREACH_EFFECT_COOLDOWN			(12 SECONDS)
/// Как часто устройство предупреждает о превышении номинала линии. Панель
/// дёргает ui_act на каждое нажатие стрелки, так что без кулдауна совет
/// превратился бы в спам и перестал читаться.
#define ATMOS_LINE_RATING_WARNING_COOLDOWN	(30 SECONDS)
/// Давление выхода, выше которого объёмный насос перестаёт качать. Ровно два
/// номинала обычной трубы - выше порога разрыва (1.8), поэтому объёмный
/// остаётся единственным насосом, штатно способным разорвать обычную линию.
/// Привязка к номиналу, а не к числу: при следующем ребалансе соотношение
/// "объёмный рвёт, газовый нет" сохранится само.
#define VOLUME_PUMP_PRESSURE_CEILING		(PIPE_PRESSURE_RATING_STANDARD * 2)

/// Давление на подающей линии, которого требуют верхние рецепты кристаллизатора.
///
/// Полтора номинала обычной трубы: держать ворота открытыми на обычной линии
/// значит держать её ровно на пороге течи - последствие вместо запрета. Достаёт
/// сюда только объёмный насос (его потолок - два номинала), а усиленная труба
/// держит с многократным запасом: контур обязан быть усиленным, но не
/// предельным. Это переворачивает механику труб: не "труба рвётся, если
/// перекачаешь", а "за усиленной трубой лежат призы, которых иначе нет".
#define GAS_RECIPE_HIGH_PRESSURE			(PIPE_PRESSURE_RATING_STANDARD * 1.5)
...
/// в игре один - "усиленный контур есть или его нет", - и двух чисел игроку
/// помнить не нужно. Обычная труба на этом давлении за полутора номиналами,
/// то есть копит усталость и уходит в течь; усиленная держит с многократным
/// запасом. Ворота работают не запретом, а последствием.
...
/// Через сколько атмос-файров прогоняется свип напряжения. При базовом
/// wait = 5 четыре файра это ровно 2 секунды - под них и посчитаны все числа
/// усталости выше. Каденция адаптивная: когда клапаны замедляют SSair,
/// деградация замедляется вместе с ним, и это правильно.
#define PIPE_STRESS_SWEEP_INTERVAL			4
...
#define ATMOS_HANDBOOK_CATEGORY_PRESSURE	"Давление в трубах"
