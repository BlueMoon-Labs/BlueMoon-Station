...
/// сорокапятикратный. Газовый насос на максимуме (4500) даёт РОВНО 100% -
/// обычная помпа, выкрученная в потолок, трубе не враг и даже не гул: сеть на
/// номинале не копит усталость и не стоит в свипе. Это осознанный баланс по
/// итогам полунедели на проде (доклады Moth и FL-432): сток ВСЕХ карт разведён
/// обычной трубой с подачей до 4500, и номинал 3000 заставлял станцию течь
/// саму по себе, без участия игрока. Порог остаётся требованием только к
/// осознанному перегрузу: объёмный насос (до 9000), нагрев контура, бомбы.
#define PIPE_PRESSURE_RATING_STANDARD		4500
...
/// набитый объёмным насосом до 9000, множит давление температурой, и потолок
/// обязан оставаться недосягаемым для всего, кроме совсем осознанного безумия.
#define PIPE_PRESSURE_RATING_REINFORCED		45000
/// Потолок, на который вправе накачивать ШТАТНОЕ мапповое оборудование: с
/// десятипроцентным запасом под номиналом обычной трубы.
///
/// Существует эта константа затем, чтобы больше никогда не разойтись с
/// номиналом молча. Сифонные вентиляторы газовых хранилищ стояли на 4000 кПа -
/// числе, которое до появления номинала не значило ничего: при номинале 3000
/// линии выхода танков на КАЖДОЙ станции начинали течь сами, без участия
/// игрока. Сейчас номинал 4000 и весь диапазон обычного насоса безопасен, но
/// правило остаётся: штатное мапповое оборудование не подходит к номиналу
/// вплотную, запас на нагрев и стакающиеся источники обязателен.
#define PIPE_SAFE_WORKING_PRESSURE			(PIPE_PRESSURE_RATING_STANDARD * 0.9)
...
/// ...и до разрыва стыка. Газовый насос упирается ровно в 1.0 и не может даже
/// продавить течь; объёмный не останавливается до 9000 кПа (2.0) и рвёт.
...
/// Давление выхода, выше которого объёмный насос перестаёт качать. Вдвое выше
/// MAX_OUTPUT_PRESSURE обычного насоса, поэтому объёмный - единственный, кто
/// штатно способен разорвать линию с обычным номиналом.
#define VOLUME_PUMP_PRESSURE_CEILING		9000

/// Давление на подающей линии, которого требуют верхние рецепты кристаллизатора.
///
/// Выше того, что обычная разводка держит без последствий (её номинал 3000, и
/// газовый насос на максимуме уже загоняет её в течь), но втрое ниже номинала
/// усиленной трубы - контур обязан быть усиленным, но не предельным. Это
/// переворачивает механику труб: не "труба рвётся, если перекачаешь", а "за
/// усиленной трубой лежат призы, которых иначе нет".
