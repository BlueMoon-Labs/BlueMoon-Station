...
/proc/add_servant_of_ratvar(mob/L, silent = FALSE, create_team = TRUE, override_type)
	...
	if(!silent && L)
		if(.)
			to_chat(L, "<span class='heavy_brass'>Мир перед вами внезапно озаряется ярко-желтым светом. [issilicon(L) ? "Вы не можете вычисилить эту истину!" : \
			...
			Ратвар, Часовой Юстициар, [GLOB.ratvar_awakens ? "освободился из своей вечной темницы" : "пребывает в изгнании, покинутый и забытый в незримом мире"].")
			...
			to_chat(L, "<span class='heavy_brass'>Мир перед вами внезапно озаряется ослепительным жёлтым светом. [issilicon(L) ? "Вы не можете вычисилить эту истину!" : \
