...
/obj/item/proc/do_pickup_animation(atom/target)
	...
	if(!direction)
		to_y += 16
	var/atom/movable/flick_visual/visual = T.flick_overlay_view(I, 6)
	var/matrix/M = new
	M.Turn(pick(-30, 30))
	animate(visual, alpha = 175, pixel_x = to_x, pixel_y = to_y, time = 3, transform = M, easing = CUBIC_EASING)
	sleep(1)
	animate(visual, alpha = 0, transform = matrix(), time = 1, flags = ANIMATION_PARALLEL)
